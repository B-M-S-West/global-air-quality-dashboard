...
class AirQualityDashboard:
    ...
    def run(self):
        ...
        # Header
        st.markdown(
            '<h1 class="main-header">🌍 OpenAQ Air Quality Dashboard</h1>',
            unsafe_allow_html=True,
        )
    ...
    def render_current_conditions(self):
        ...
        # Get latest measurements
        latest_data = self.client.get_latest_measurements(
            countries=[st.session_state.get("selected_country")]
        ...
        for param, measurements in param_data.items():
            ...
            with st.expander(f"{display_name} - {len(measurements)} measurements"):
                ...
                values = [
                    m["value"] for m in measurements if m.get("value") is not None
                ]
                if values:
                ...
                st.markdown(
                    f'<div class="pollutant-info"><strong>About {display_name}:</strong> {description}</div>',
                    unsafe_allow_html=True,
                )
            ...
            if available_columns:
                ...
                if "parameter" in df_display.columns:
                    df_display["parameter"] = df_display["parameter"].map(
                        lambda x: POLLUTANT_INFO.get(x, {}).get(
                            "display_name", x.upper()
                        )
                    )
    ...
    def render_historical_trends(self):
        ...
        location_options = {
            f"{loc.get('name', 'Unknown')}": loc["id"]
            for loc in locations[:50]
        }  # Limit for performance

        selected_location_name = st.selectbox(
            "Select Location for Detailed Analysis", list(location_options.keys())
        )
        ...
        if not pollutant_data:
            st.warning(
                "No historical data available for the selected location and time range."
            )
            return

        # Display options
        chart_type = st.selectbox(
            "Select Chart Type", ["Individual Pollutants", "Multi-Pollutant Comparison"]
        )

        if chart_type == "Individual Pollutants":
            # Individual charts for each pollutant
            for param, data in pollutant_data.items():
                chart = self.charts.create_time_series_chart(
                    data, param, selected_location_name
                )
            ...
            if len(pollutant_data) > 1:
                chart = self.charts.create_multi_pollutant_chart(
                    pollutant_data, selected_location_name
                )
                st.plotly_chart(chart, use_container_width=True)
            else:
    ...
    def render_location_comparison(self):
        ...
        comparison_chart = self.charts.create_comparison_chart(
            locations_data, comparison_param
        )
