...
class AirQualityCharts:
    ...
    @staticmethod
    def create_time_series_chart(data: List[Dict], parameter: str, 
                               location_name: str = "Location") -> go.Figure:
    ...
    @staticmethod
    def create_multi_pollutant_chart(data_dict: Dict[str, List[Dict]], 
                                   location_name: str = "Location") -> go.Figure:
        """Create chart comparing multiple pollutants."""
        fig = make_subplots(
            rows=len(data_dict), cols=1,
            subplot_titles=[POLLUTANT_INFO.get(param, {}).get('display_name', param.upper()) 
    ...
    @staticmethod
    def create_comparison_chart(locations_data: Dict[str, Dict[str, List[Dict]]], 
    ...
    def create_aqi_gauge(value: float, parameter: str = 'pm25') -> go.Figure:
        ...
        fig = go.Figure(go.Indicator(
            ...
            value=value,
            domain={'x': [0, 1], 'y': [0, 1]},
            title={'text': f"{POLLUTANT_INFO.get(parameter, {}).get('display_name', parameter.upper())} AQI"},
