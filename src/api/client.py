...
class OpenAQClient:
    ...
    def __init__(self):
        ...
        self.session.headers.update(
            {"X-API-Key": self.api_key, "User-Agent": "OpenAQ-Python-Dashboard/1.0"}
        )
    ...
    def get_latest_measurements(
            ...
            if location_ids:
                # Build params dict for filtering
                params = {"limit": 1000}
                if countries:
                ...
                # For specific locations, call each location's latest endpoint
                for location_id in location_ids:
                    endpoint = f"locations/{location_id}/latest"
                    data = self._make_request(endpoint, params)  # Pass params here!
                    results.extend(data.get("results", []))
            
            elif parameters:
                # For specific parameters across all locations
                param_data = self.get_parameters()
                param_map = {p.get('name'): p.get('id') for p in param_data}
                ...
                st.warning("Getting all latest measurements requires fetching all locations first")
                return []
    ...
    def get_location_measurements_aggregated(
        self,
        location_id: int,
        parameter: str,
        period: str = "hour",
